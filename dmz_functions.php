...
function doBuyStock($symbol, $amount) {
	...
	$total = round($t,2);
	//Now cURL for the portfolio values
	$ch = curl_init("https://www.alphavantage.co/query?function=TIME_SERIES_DAILY&symbol=" .$symbol. "&apikey=9J4N8FA67HVHYZG0");
	curl_setopt($ch, CURLOPT_HEADER, 0);
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, TRUE);
	$curl_results = curl_exec($ch);
	curl_close($ch);
	...
	echo $old_stock;
	echo "\n";
	$buyarray = array($i_amount, $total, $old_stock, $cur_stock);
